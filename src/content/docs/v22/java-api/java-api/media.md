...
Class BaseClip java.lang.Object | +--org.kwis.msp.media.BaseClip public abstract class BaseClip extends java.lang.Object 매체 처리기를 구현하기 위한 최상위 추상화 클래스이다. Methods inherited from class java.lang.Object clone, equals, finalize, getClass, hashCode, notify, notifyAll, toString, wait, wait, wait 메쏘드 상세설명

#### allocPlayer

protected int allocPlayer() 클립내의 미디어 데이타를 처리하기 위한 플레이어를 할당 받는다. 이 메소드는 클립의 미디어 데이타를 이용하여 재생하는 등의 실질적인 처리를 하기 이전에 반드시 불리워져야만 하는 메소드이다. 만약 이 메소드를 호출 하지 않고, 혹은 이 메소드가 실패 한 후에, play 등의 메소드가 불리워지면 에러값을 반환 받게 된다.
...
성공하면 0을 반환하고, 실패하면 음수 반환 freePlayer protected int freePlayer() allocPlayer() 메소드를 이용해서 할당 받았던 플레이어를 해제 시킨다.
...
성공하면 0을 반환하고, 실패하면 음수 반환 free public void free() 클립 내의 사용한 모든 리소스를 해제한다. 이 메소드를 호출하지 않으면 사용된 클립 데이터나 리소스는 메모리에 남아 있을 수 있다.
...
없음 mediaControl protected int mediaControl(int cmd, int[] buf1, int[] buf2) 매체의 일반적인 기능 이외에 제조사에서 지원해주는 특별한 기능 명령을 수행 시킬 때 사용되어 진다. 예를 들면, 제조사에서 현재 재생시간을 얻어올 수 잇는 제어 명령을 지원한다면, 그 명령을 이 함수를 이용해서 내릴 수 있다. 매개변수에 따라 타입이 맞는 메소드를 사용하면 된다.
...
protected int mediaControl(int cmd, int[] buf1, byte[] buf2) 매체의 일반적인 기능 이외에 제조사에서 지원해주는 특별한 기능 명령을 수행 시킬 때 사용되어 진다. 예를 들면, 제조사에서 현재 재생시간을 얻어올 수 잇는 제어 명령을 지원한다면, 그 명령을 이 함수를 이용해서 내릴 수 있다. 매개변수에 따라 타입이 맞는 메소드를 사용하면 된다.
...
성공 0 실패 음수 참고사항 [표 2-17-3]에 정의된 것 중 이 메쏘드에서 사용할 수 있는 제어 명령은 다음과 같다. MDACTRL_GET_CAPTURE_IMAGE mediaControl protected int mediaControl(int cmd, String buf1, int[] buf2) 매체의 일반적인 기능 이외에 제조사에서 지원해주는 특별한 기능 명령을 수행 시킬 때 사용되어 진다. 예를 들면, 제조사에서 현재 재생시간을 얻어올 수 잇는 제어 명령을 지원한다면, 그 명령을 이 함수를 이용해서 내릴 수 있다. 매개변수에 따라 타입이 맞는 메소드를 사용하면 된다.
