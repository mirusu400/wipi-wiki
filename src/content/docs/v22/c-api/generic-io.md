...
```c
typedef void (*DEVCONTROLCB)(M_Int32 fd, M_Int32 error, M_Char *cmd,
M_Uint32 param_sz, void *param)
```

**설명**

함수 `MC_ioDevSetControlCB` 에 등록하는 콜백 함수. MC_ioDevControl함수에 대한 반환값을 즉시 반환 받을 수 없을 경우, 해당 장치가 non-blocking으로 동작할 경우 에 OEM에서 플랫폼을 통해 반환되는 시점에서 불린다.

**매개 변수**

- `fd` - 장치 식별자
- `error` - 디바이스별 추가되는 서브이벤트.
- `cmd` - `MC_ioDevControl` 호출 시 사용했던 컨트롤 커맨드. (본 파라메터를 통해 Application은 어떤 커맨드에 대한 Callback 호출인지 판단한다.)
- `param_sz` - param 사이즈
- `param` - 콜백 함수를 등록할 때 설정하는 콜백 매개 변수
...
- `M_E_INPROGRESS` - 해당 장치가 연결을 시도 중인 경우
- `M_E_ISCONN` - 해당 장치가 이미 열려 있는 경우
- `M_E_ERROR` - 기타 에러
- `M_E_INVALID` - 장치이름이 잘못되었을 경우, 유효하지 않은 장치번호일 경우(0보다 작거 나 지정된 장치번 호 개수보 다 크거나 같을 경우), param 매개 변수가 잘못된 경우(디바이스 특성에 따름)
...
```c
M_Int32 MC_ioDevSetControlCB(M_Int32 fd, DEVCONTROLCB cb)
```

**설명**

Non-blocking I/O 장치의 경우 해당 장치를 통해서 `MC_ioDevControl` API 의 처리 결 과를 전송할 수 있는 시점에서 불리는 콜백 함수를 등록한다. 콜백 함수가 불리면 DEVCONTROLCB 의 매개 변수인 error, cmd, param_sz, param값이 콜백 함수 API 로 전달된다. 등록하는 콜백 함수가 NULL일 경우 이전에 이 함수로 등록된 콜백 함 수가 삭제된다. 이 함수가 에러를 반환하면 콜백 함수는 불리지 않는다.
...
- `param` - 설정하려는 모드는 아래의 `MC_IrDAMode` 구조체를 이용하여 전달한다. 서버 모드일 경우에는 aszMode에 “Server”를, 클라이언트 모드일 경우에는 aszMode에 “Client”를 넣어 전달한다. nTime은 연결을 시도할 시간을 명시한다. (nTime이 0이면 연결이 성립될 때 까지 시도한다. nTime이 0으로 셋팅된 경우, 단말이 설정한 연결최대시간 동안 연결을 시도한다.) 타임 아웃이 발생한 경우에는 `MC_ioDevSetOpenCB()` API를 사용하여 등록한 콜백함수에 M_E_TIMEOUT이 전달된 다.
...
- `M_E_ERROR` - 기타 에러
- `M_E_INVALID` - 장치이름이 잘못되었을 경우, 유효하지 않은 장치번호일 경우(0보다 작거 나 지정된 장치번 호 개수보 다 크거나 같을 경우), param 매개 변수가 잘못된 경우(aszMode가 잘못된 경우) 이며 디바이스 특성에 따름
...
- `M_E_INVALID` - 장치이름이 잘못되었을 경우, 유효하지 않은 장치번호일 경우(0보다 작거 나 지정된 장치번 호 개수보 다 크거나 같을 경우), param 매개 변수가 잘못된 경우
- `M_E_ERROR` - 기타 에러
