...
SecurityException 파일을 생성한다. 파일이름은 구분자("/")로 시작하는 것과 없이 바로 파일이름으로 시작하는 것과 차이가 없다. 파일을 생성할 때는 다음과 같은 모드로 열 수 있다. READ_ONLY read only로 읽다. 만약 쓰려고 하면 exception이 발생한다. WRITE 현재 파일을 유지하고 파일 포인터가 제일 뒤에 위치하게 된다. WRITE_TRUNC 파일이 truncation된다. 즉, 파일을 여는 순간 파일 길이는 0으로 된다. READ_WRITE read와 write를 동시에 할 수 있다. 위 모드 외에 다른 모드로 열고자 하면 IllegalArgumentException을 발생시킨다.
...
DataInputStream을 가져온다. 열 수 있는 DataInputStream개수는 InputStream포함해 서 최대 한 개이다. Specified by openDataInputStream in interface InputConnection
...
OutputStream을 가져온다. 열 수 있는 OutputStream개수는 DataOutputStream포함 최 대 한 개이다. Specified by openOutputStream in interface OutputConnection
...
DataOutputStream을 가져온다. 열 수 있는 DataOutputStream개수는 OutputStream포 함해서 최대 한 개이다. Specified by openDataOutputStream in interface OutputConnection
...
- `b` - 쓸 한 바이트 Throws
- `IOException` - close함수로 닫혀진 File에서 write하는 경우이거나, 제대로 쓸 수 없을 경우 write
...
NullPointerException 파일에 buf에 들어 있는 데이타를 buf의 길이만큼 쓴다. buf가 null일 경우에
- `NullPointerException을` - 발생시킨다.

**매개 변수**

- `buf` - 실제 데이타가 들어있는 byte array Throws
- `IOException` - close함수로 닫혀진 File에서 write하는 경우이거나, 제대로 쓸 수 없을 경우
- `NullPointerException` - buf가 null인 경우 write
...
NullPointerException 파일에 buf에 들어 있는 데이타를 off부터 시작해서 len만큼 쓴다. buf가 null일 경 우에 NullPointerException을 발생시킨다.

**매개 변수**

- `buf` - 실제 데이타가 들어있는 byte array
- `off` - 쓸 데이타가 들어있는 위치
- `len` - 실제로 쓸 데이타 크기 Throws
- `IOException` - close함수로 닫혀진 File에서 write하는 경우이거나, 제대로 쓸 수 없을 경우
- `NullPointerException` - buf가 null인 경우 read
...
- `input` - stream으로 부터 데이타를 읽어 들이다. buf의 size만큼 읽어 들이다. 만약 파일의 끝까지 모두 읽었을 경우 그 읽은 만큼만 buf에 저장하고, 저장된 개수를 돌 려준다. 그리고, 다음 read시에 -1을 돌려준다.
...
NullPointerException input stream으로 부터 데이타를 len 바이트 수만큼 읽어 들이다. len 만큼 읽지 못 하고 EOF를 만나면 읽은 바이트 수만큼을 buf에 저장해서 돌려준다.
...
- `IOException` - 파일 handle이 제대로 세팅되지 않았거나 파일 포인터를 옮기 는 도중 에러가 발생했을 경우 sizeOf
...
파일의 크기 Throws IOException 파일 handle이 제대로 세팅되지 않았거나 크기를 읽어 오는 도 중 에러가 발생했을 경우
...
dirname아래에 있는 모든 파일이름과 디렉토리 이름 Throws IOException 디렉토리가 존재하지 않을 경우
...
SecurityException 지정된 디렉토리 안에 있는 파일과 디렉토리를 모두 보여준다. dirname이 null일 경 우 NullPointerException을 발생시킨다. dirname이 MAX_FILENAME_LENGTH보다 길 경 우 IOException을 발생시킨다.
...
dirname아래에 있는 모든 파일 이름과 디렉토리 이름 Throws SecurityException 허가되지 않은 디렉토리를 접근하려고 할 경우. IOException dirname이 존재하지 않을 경우
...
SecurityException 지정된 디렉토리아래 파일이나 디렉토리가 존재하는 지를 확인한다. name이 null일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다.
...
존재하면 true, 존재하지 않으면 false Throws SecurityException 접근할 수 없는 디렉토리를 접근하려고 할 경우
...
- `IOException` - 파일을 지울 때 제대로 지우지 못할 경우, 파일이 존재하지 않을 경우 remove
...
SecurityException 파일을 지운다. filename이 null일 경우 NullPointerException을 발생시킨다. 파일 이 존재하지 않아도 exception을 발생하지 않는다. filename이 MAX_FILENAME_LENGTH 보다 길 경우 IOException을 발생시킨다.
...
- `SecurityException` - 접근할 수 없는 디렉토리를 접근하려고 할 경우
- `IOException` - 파일을 지울 때 제대로 지우지 못할 경우, 파일이 존재하지 않을 경우 mkdir
...
- `NullPointerException을` - 발생시킨다. dirname이 MAX_FILENAME_LENGTH보다 길 경우
- `IOException을` - 발생시킨다.

**매개 변수**

- `dirname` - 디렉토리 이름 Throws
- `IOException` - 디렉토리를 만들 수 없을 경우나 디렉토리가 이미 존재할 경우, 또는 파일 이름 길이가 최대값을 넘어 갈 경우 mkdir
...
SecurityException 디렉토리를 만든다. dirname이 null일 경우 NullPointerException을 발생시킨다.
...
- `flag` - 접근하고자 하는 디렉토리 Throws
- `SecurityException` - 접근할 수 없는 디렉토리를 접근하려고 할 경우
- `IOException` - 디렉토리를 만들 수 없을 경우나 디렉토리가 이미 존 재할 경우, 또는 파일 이름 길이가 최대값을 넘어 갈 경우 rmdir
...
- `dirname` - 디렉토리 이름 Throws
- `IOException` - 디렉토리가 비어있지 않거나 디렉토리를 지울 수 없을 경우, 또는 디렉토리가 존재하지 않을 경우 rmdir
...
SecurityException 디렉토리를 지운다. dirname이 null일 경우 NullPointerException을 발생시킨다.
- `dirname이` - MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다.
...
- `flag` - 접근하고자 하는 디렉토리 Throws
- `SecurityException` - 접근할 수 없는 디렉토리를 접근하려고 할 경우
- `IOException` - 디렉토리가 비어있지 않거나 디렉토리를 지울 수 없을 경우, 또는 디렉토리가 존재하지 않을 경우 toCString
...
```java
public static boolean isFile(String name) throws IOException
```

응용프로그램 자신만의 디렉토리에 지정된 이름의 파일이 존재하는지 확인한다. name이 null일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다. 같은 디렉토리 아래에 같은 이름의 파일과 디렉토리는 존재할 수 없다.
...
name과 같은 파일이 존재하면 true, 디렉토리 이름이거나 존재하지 않으면 false
...
SecurityException 지정된 이름의 파일이 존재하는지 확인한다. name이 null일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다. 같은 디렉토리 아래에 같은 이름의 파일과 디렉토리는 존재할 수 없다.
...
name과 같은 파일이 존재하면 true, 디렉토리 이름이거나 존재하지 않으면 false Throws SecurityException 접근할 수 없는 디렉토리를 접근하려고 할 경우
...
응용프로그램 자신만의 디렉토리에 있는 디렉토리인지를 확인한다. 같은 디렉토리 아래에 같은 이름의 파일과 디렉토리는 존재할 수 없다. name이 null일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다.
...
name과 같은 디렉토리 이름이 존재하면 true, 파일이름이거나 존재하지 않으 면 false
...
SecurityException 디렉토리인지를 확인한다. 같은 디렉토리 아래에 같은 이름의 파일과 디렉토리는 존 재할 수 없다. name이 null일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다.
...
name과 같은 디렉토리 이름이 존재하면 true, 파일이름이거나 존재하지 않으 면 false Throws SecurityException 접근할 수 없는 디렉토리를 접근하려고 할 경우
...
응용프로그램 자신만의 디렉토리에 있는 파일의 생성시간을 알아온다. name이 null 일 경우 NullPointerException을 발생시킨다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다. name이 만약 디렉토리일 경우는 -1을 return한다.
...
만약 디렉토리이면 -1을, file일 경우는 생성시간 초로 표현
...
SecurityException 파일의 생성시간을 알아온다. name이 null일 경우 NullPointerException을 발생시킨 다. name이 MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다. name이 만 약 디렉토리일 경우는 -1을 return한다.
...
- `IOException` - 바꾸는 과정에 에러가 발생할 경우 rename
...
SecurityException 파일 이름을 바꾼다. 바꾸고자 하는 파일의 현재 이름과 바뀐 후 이름은 디렉토리를 포함하는 경로가 될 수 없다. 즉 디렉토리 구분자가 포함되어서는 안 된다. oldName 이나 newName이 null일 경우 NullPointerException을 발생시킨다. oldName이나
- `newName이` - MAX_FILENAME_LENGTH보다 길 경우 IOException을 발생시킨다.
