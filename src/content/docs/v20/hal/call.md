...
전화를 걸거나 받는 함수들이다. 플랫폼 수행 중에 전화가 걸려 올 때 수신 여부를 결정할 수 있고, 플랫폼에서 전화를 걸 수 있다. 운영체제는 `MH_pltEvent()` 함수에 매개변수로 `MH_CALL_EVENT` 와 `MH_CallEvent` 를 넘겨 주어 플랫폼에 이벤트를 전달한다
