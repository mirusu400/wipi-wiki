...
`MH_termResGetCount`
단말 리소스 그룹 관리 `MH_termResGetList`
...
단말 리소스 기타 `MH_termResExecuteCmd`
다 단말 리소스 그룹
...
어플리케이션이 폰북개인을 write할 때 폰북그룹 리스트에 없는 폰북그룹을 설 정하면 에러를 반환하여야 한다. 어플리케이션이 폰북개인에 폰북그룹을 설정하 면, 해당 폰북그룹에 속해있는 폰북개인 리스트 정보도 update되어야 한다. 자 단말 리소스 보안 기능
...
- `resGroup` - [out] 리소스 그룹 리스트(Terminal Resource 모듈 포팅의 ‘리스트 형식’을 따른다.)
...
- `resGroupName` - [in] 리소스 그룹 이름
- `aszList` - [out] 리소스 이름 리스트 (Terminal Resource 모듈 포팅의 ‘리스트 형식’을 따른다.)
...
MH_termResRead를 호출해서 리소스를 읽어올 때 필요한 버퍼를 할당하기 위 해서 사용된다.
...
pData는 `MH_termResGetSize`()를 통해 얻어온 리소스의 크기만큼 Caller가 할 당한다.
...
M_Int32 nameSize, M_Char* uiName, M_Char* resFormat, M_Byte* pData, M_Int32 bufSize, M_Int32 mode)
```
...
- `pData` - [in] 리소스 데이터
- `bufSize` - [in] pData 버퍼 크기
- `mode` - [in] 쓰기 모드 `MH_TERMRES_CREATE` 리소스를 새로 생성한다. 이 경우 resName은 새로 생성된 리소스 이름이 반환된다. `MH_TERMRES_UPDATE` 기존 리소스에 덮어 쓴다. 이 경우 resName에 덮어 쓸 리소스 이름을 넣어준다. 복수개의 리소스를 WRITE할 경우 resName, uiName, pData 인자의 각 데이타는 ₩0 문자로 구분하고 마지막엔 ₩0문자 2개가 온다
...
`MH_TERMRES_CREATE` 모드시, OEM에서 UI이름에 대한 특별한 규칙이 있 거나 UI이름이 없는 리소스 그룹의 경우, 인자로 넘겨준 UI이름은 무시될 수 있다. `MH_TERMRES_UPDATE` 모드시, 리소스 그룹 이름, 리소스 이름, MIME 타입 이 존재하는 리소스의 정보와 모두 일치하는 경우에 함수가 동작하며 기존 데이터는 새로운 데이터로 업데이트된다. UI이름을 지원하는 리소스 그룹일 경우 UI이름을 지정해야 하며, 그렇지 않을 경우 에러(`M_E_INVALID`)를 반환해야 한다.

**참고 항목**

PHONEBOOK/SHORTKEY와 PHONEBOOK/GROUP은 UI 이름과 리소스 이름 이 같아야 한다.
...
- `resList` - [out] 리소스 그룹과 리소스 이름의 리스트 리소스 그룹 이름, “;”, 리소스 이름 순으로 구성된다. 리소스가 두개일 경우 다음의 리소스 그룹이름과 리소스 이름의 구별은 Terminal Resource 모듈 포팅의 ‘리스트 형식’을 따른다. 예) “MUSICBELL;mymusicbell₩0₩0” “MUSICBELL;mymusicbell₩0PICTUREMATE;mypicture₩0₩0”
...
```c
M_Int32 MH_termResSetGroupLockState(M_Char* resGroupName,
M_Int32 state)
```
...
MH_termResCheckPassword를 이용해 비밀번호를 확인한 후 사용한다.
...
```c
M_Int32 MH_termResGetFreeSpace(M_Char* resGroupName)
```

**설명**

단말 리소스 저장 공간의 남은 크기를 반환한다.
...
단축번호로 사용할 수 있는 단말기)
PHONEBOOK/GRO “IRREMOVABLE 삭제가 불가능한 리소스 리스트를 반환한다. 반환
UP ” 형식은 Terminal Resource 모듈 포팅의 ‘리스트 형
식’을 따른다. 삭제 불가능한 리소스가 없을 경우
에러(`M_E_NOENT`)를 반환한다.
‘PHONEBOOK/PRI “MAXTELCOUN 단말에 저장 가능한 최대 전화번호 수를 문자열로
VATE T” 반환한다.
“TYPELIST” 단말이 지원하는 wCard Type의 리스트를 반환한다.
반환 형식은 Terminal Resource 모듈 포팅의 ‘리스
트 형식’을 따른다.
...
예2) “TYPEINFO/TEL”을 infoType으로 하여 “15/4/cell:4/work:4/home:4”
반환
...
설명 : 하나의 폰북개인에 집전화, 회사전화, 휴대전화 번호를 통틀어 4개 저
장할 수 있다. 4개 모두를 집전화번호로 설정할 수도 있으며, 4개 모두를 회사전화번
호 또는 휴대전화번호로도 설정할 수 있다.
...
‘X-MDAY’ Type에는 birthday, wedding, meeting, memorial, sun, moon 6개의
Type parameter가 있다.
Type parameter birthday, wedding, meeting, memorial은 하나의 폰북개인에 각
각 1개씩 저장 가능.
Type parameter sun, moon은 하나의 폰북개인에 각각 4개씩 저장 가능.
설명 : 하나의 폰북개인에 기념일은 모두 4개 저장할 수 있으며, 생일, 결혼기
념일, 만남, 기념은 각각 하나씩 저장할 수 있다. 따라서 생일, 결혼기념일, 만남, 기
념을 각각 하나씩 저장하여 4개의 기념일을 저장하게 된다. 4개의 기념일 모두 양력
으로 저장하거나 음력으로 저장할 수 있다.
...
‘+’ : 앞뒤 문자열/문자를 strcat 하는 연산을 의미한다. 리소스 포맷은 Terminal Resource 모듈 포팅의 ‘리소스 데이터 포맷 및 MIME 타입’을 따른다.
...
sec)이다.
BTSINFO 포맷 “MAXCOUNT” 지정한 리소스가 가질 수 있는 최대 정보의 개
수
POWERONOFF 포맷
...
이름 리스트를 반환한다. 반환 형식은 Terminal
Resource 모듈 포팅의 ‘리스트 형식’을 따른다.
...
```c
M_Int32 MH_termResSearch (M_Char* resGroupName, M_Char* queryType, M_Char* queryName, M_Byte* resNames, M_Int32 bufSize,
M_Int32 mode)
```
...
- `resNames` - [out] 리소스 이름 리스트. (Terminal Resource 모듈 포팅의 ‘리스트 형식’을 따른다.)
...
<2-10-14> 리소스 그룹 및 질의 타입 리소스 그룹 queryType 비고 모든 리소스 그룹 “UINAME” 지정한 리소스 그룹에서 queryName 문자 열을 가진 UI 이름의 리소스 검색 (UI이름 을 지원하지 않는 리소스 그룹이 있을 수 있음) PHONEBOOK/PRIVATE “NUMBER” 전화번호 검색 : 전화번호에 queryName 문자열을 가진 폰북개인 검색 “PCARD” 사진명함인덱스를 가진 폰북개인 검색. queryName과 mode는 무시한다.
...
지정 가능한 커맨드는 다음과 같으며 차후 추가될 수 있다.
