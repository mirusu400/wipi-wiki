...
```c
typedef  void  (*NETHTTPCB)(M_Int32  fd,  M_Int32  sd, M_Int32 error, void
*param)
```
...
M_E_ISCONN – 이 어플리케이션이 이미 인터넷 접근이 허용된 경우 M_E_ERROR – 기타 에러가 발생했을 경우
...
- `domain` — 통신 도메인으로 인터넷 소켓일 경우 AF_INET 임 type - MC_SOCKET_STREAM 혹은 MC_SOCKET_DGRAM
...
M_E_NOTSUP – domain 혹은 type 이 지원하지 않는 값을 경우 M_E_NOTCONN - 인터넷 접근이 허용되지 않은 경우 M_E_NOSPACE – 더 이상 소켓을 생성할 수 없을 경우 M_E_ERROR – 기타 에러

```c
MC_netSocketConnect 설명
```
...
M_E_NOTSUP – 이 함수를 지원하지 않는 소켓일 경우 M_E_BADFD - 유효하지 않는 소켓 식별자 M_E_NOTCONN - 인터넷 접근이 불가능한 경우 M_E_ISCONN - 이미 연결되어 있는 경우 M_E_INPROGRESS - 연결 작업이 이미 진행 중인

M_E_INVALID – 주소가 잘 못된 경우나 콜백함수가 NULL 일 경우 M_E_ERROR – 기타 에러

```c
MC_netSocketClose
MC_netSocketWrite 설명
```
...
M_E_NOTCONN – 서버와 소켓 연결이 끊기거나 인터넷 접근이 불가능한 경 우
...
M_E_WOULDBLOCK - 데이터가 도착하지 않아 지금 읽을 수 없는 경우 M_E_NOTCONN –서버와 서켓 연결이 끊기거나 인터넷 접근이 불가능한 경우 M_E_BADFD - 소켓 식별자가 유효하지 않은 경우

M_E_INVALID – 버퍼나 버퍼길이가 잘 못된 경우

```c
MC_netSetReadCB
MC_netSocketClose 설명
```
...
M_E_INVALID - 이미 특정포트가 다른 소켓을 위해 지정되어 있는 경우 M_E_NOTCONN - 인터넷 접근이 불가능한 경우

M_E_BADFD - 유효하지 않은 소켓 식별자인 경우

```c
MC_netGetMaxPacketLength 설명
```
...
- `fd` — 소켓 식별자 buf - 데이터의 버퍼 len - 데이터의 길이
- `addr` — 상대방 IP 주소. 네트웍 바이트 순서(network byte ordering)이다. port - 상대방 포트번호. 네트웍 바이트 순서(network byte ordering)이다.
...
M_E_BADFD - 유효하지 않는 소켓 식별자 M_E_NOTCONN - 인터넷 접근이 불가능한 경우

M_E_INVALID – 주소 혹은 버퍼나 버퍼 길이가 잘 못 된 경우

```c
MC_netGetMaxPacketLength MC_netSetWriteCB
MC_netSocketRcvFrom 설명
```

UDP 소켓을 통해 데이타를 읽어 온다.

데이타를 전송한 상대방의 IP 주소와 포트번호는 각각 addr 와 port 에 저장된다. 상대방의 IP 주소와 포트번호는 모두 네트웍 바이트 순서(network byte ordering) 이다. 이 함수가 M_E_WOULDBLOCK 을 리턴하면 시스템 내부 요인으로 지금 데이타 를 읽을 수 없다는 뜻이다. 이 경우 MC_netSetReadCB() 로 등록하는 콜백함수 내 에서 다시 이 함수를 불러 데이터를 읽는다. 콜백내에서 부른 이 함수는 다시 M_E_WOULDBLOCK 을 리턴할 수 있다.
...
M_E_WODULDBLOCK - 도착한 데이터가 없어 지금 당장 데이터를 읽을 수 없 은 경우

M_E_BADFD - 유효하지 않는 소켓 식별자 M_E_NOTCONN - 인터넷 접근이 불가능한 경우

M_E_INVALID – 데이터 버퍼나 버퍼길이 혹은 상대방 IP 와 포트가 저장될 버퍼가 잘 못된 경우

```c
MC_netSetReadCB
MC_netGetHostAddr 설명
```
...
```c
dnsserver – 도메인 네임 서버 IP 주소 hostname –호스트 이름
cb – 호스트 이름에 해당하는 IP 주소를 얻게 되면 호출되는 콜백함수 param – 콜백함수가 호출될 때 전달되는 매개변수
```
...
0 혹은 매개변수 hostname 이 NULL 일 경우 M_E_ERROR – 기타 에러
...
M_E_WOULDBLOCK – 지금 클라이언트와 연결된 소켓이 없을 경우 M_E_NOTSUP – 이 함수를 지원하지 않는 소켓일 경우 M_E_BADFD – 잘 못된 식별자일 경우

M_E_NOTCONN –인터넷 접근이 불가능한 경우 M_E_ERROR - 기타 이유로 실패할 경우
...
M_E_NOTCONN –인터넷 접근이 불가능한 경우 M_E_BADF - 소켓 식별자가 잘 못 되었을 경우
