...
M_E_INUSE : 클립을 재생중이거나 녹음중에 해제할려고 시도함 M_E_INVALID: clip 이 NULL 이면 리턴

```c
MC_mdaClipGetType 설명
```
...
```c
buf – [out] 타입이 저장될 버퍼 bufSize – [in] 복사할 버퍼 크기
```
...
```c
filename – [in] 복사해올 파일 이름 size – [in] 복사할 버퍼 크기
```
...
```c
MC_mdaClipAvailableDataSize 설명
```
...
```c
MC_mdaClipSetPosition 설명
```

재생을 시작할 위치를 설정한다. 재생위치 설정기능을 지원하지 않는 타입으로 생 성된 클립에 이 함수를 호출할 경우, M_E_NOTSUP 가 반환된다.
...
M_E_INUSE – 클립재생기가 이미 다른 클립을 재생하고 있음 M_E_ERROR – 이미 같은 클립을 재생중에 있음
...
녹음을 지원하지 않는 타입으로 생성된 클립으로 녹음을 시도할 경우, 아무기능도 하지 않는다. 이 함수가 불려 매체처리를 시작하면 클립생성시 등록된 콜백 함수 에 MC_MDA_STATUS_RECORDED 상태가 전달된다. 이미 함수가 불려 녹음중이었다면, 이 함수는 아무런 역할을 하지 않는다. 녹음 중, 클립 내부버퍼가 완전히 차면 콜 백함수에 MC_MDA_STATUS_FULL 상태가 전달된다.
...
진동세기가 강, 약 두개인 하드웨어 => 1-50 : 약진동, 51-100 : 강진동 진동세기가 강,중,약 세개인 하드웨어 => 1-33:약진동, 34-66:중진동 , 67-100:강
진동
```
...
MC_MDA_VOLTYPE_RECORDER 중의 하나가 될 수 있다 bmute – [in] 소리발생 방지 설정
TRUE : 소리 발생 방지
FALSE : 소리 발생 허용
```
