...
```c
typedef enum _MC_BackLight MC_BackLight { MC_LIGHT_ON = 0	// 백라이트 켬
MC_LIGHT_OFF,	// 백라이트 끔 MC_LIGHT_ALWAYS_ON,	// 백라이트 항상 켬 MC_LIGHT_DEFAULT,	//사용자가 설정한 상태로 함
} MC_BackLight;
```
...
- `id` — 백라이트 번호
- `on_off` — 백라이트 조정옵션 color – 백라이트 색상
...
LED on/off 를 설정한다. 각 bit 가 1 이면 on, 0 이면 off 를 나타낸다. 예)
...
M_Int32 타입의 값의 Network Byte Odering 을 Host Byte Odering 으로 전환한다.
...
M_Int16 타입의 값의 Network Byte Odering 을 Host Byte Odering 으로 전환한다.
...
```c
MC_utilInetAddrStr 설명
```

정수형의 IP 값으로 부터 IP 문자열을 얻다. 정수형 값은 Network Byte Odering 이어 야 한다.
